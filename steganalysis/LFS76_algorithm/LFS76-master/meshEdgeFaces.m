...
%             b1 =(sum(abs(edges-ones(length(edges),1)*currentEdge).^2,2)==0);
            b1=find((abs(edges(:,1)-currentEdge(1,1))+abs(edges(:,2)-currentEdge(1,2)))==0);
%             b1=(~(edges(:,1)-repmat(currentEdge(1,1),length(edges),1)))&(~(edges(:,2)-repmat(currentEdge(1,2),length(edges),1)));
            indEdge = b1;
            if ~isempty(indEdge)
...
%             b2 =(sum((edges-ones(length(edges),1)*currentEdge([2 1])).^2,2)==0);
            b2=find((abs(edges(:,1)-currentEdge(1,2))+abs(edges(:,2)-currentEdge(1,1)))==0);
%             b2=(~(edges(:,1)-repmat(currentEdge(1,2),length(edges),1)))&(~(edges(:,2)-repmat(currentEdge(1,1),length(edges),1)));
            indEdge = b2;
            if ~isempty(indEdge)
